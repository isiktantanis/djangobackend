    ...
    if request.method == "GET":
        ...
        if "nft" in reqData.keys():
            ...
            if "DAY" in reqData.keys():
                transQuery = transQuery.filter(
                    time__year=currentTime.year,
                    time__month=currentTime.month,
                    time__day=currentTime.day,
                )
            elif "MONTH" in reqData.keys():
                transQuery = transQuery.filter(
                    time__year=currentTime.year, time__month=currentTime.month
                )
            elif "YEAR" in reqData.keys():
                transQuery = transQuery.filter(time__year=currentTime.year)
            valueSet = (
                transQuery.values("nft").order_by().annotate(nft__count=Count("nft"))
            )
            print(valueSet)
        ...
        if "user" in reqData.keys():
            ...
            if "DAY" in reqData.keys():
                ...
                transQuery = transQuery.filter(time__year=currentTime.year)
            valueSet = (
                transQuery.values("oldOwner")
                .order_by()
                .annotate(oldOwner__count=Count("oldOwner"))
            )
        ...
        if "collection" in reqData.keys():
            ...
            if "DAY" in reqData.keys():
                ...
                transQuery = transQuery.filter(time__year=currentTime.year)
            valueSet = (
                TransHist.objects.values("nft")
                .order_by()
                .annotate(nft__count=Count("nft"))
            )
